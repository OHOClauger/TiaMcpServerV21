...
namespace TiaMcpServer.Siemens
{
    ...
    public class Engineering
    {
        ...
        private static string? GetTiaPortalInstallPath()
        {
            ...
            {
                // V20+ uses EditionMain\Path; older versions used TIA_Opns\Path
                var subKeyNames = new[]
                {
                    $@"SOFTWARE\Siemens\Automation\_InstalledSW\TIAP{TiaMajorVersion}\TIA_Opns",
                    $@"SOFTWARE\Siemens\Automation\_InstalledSW\TIAP{TiaMajorVersion}\EditionMain",
                };

                foreach (var subKeyName in subKeyNames)
                {
                    using (var key = regBaseKey.OpenSubKey(subKeyName))
                    {
